...
class Map {
  constructor(mapId, opts) {
    ...
    let centerPos = ol.proj.transform(opts.center, 'EPSG:4326', 'EPSG:3857')
    // 创建地图
    let map = new ol.Map({
    ...
    let tileLayer = new ol.layer.Tile({
      source: new ol.source.XYZ({
        tileUrlFunction: function (coordinate) {
          let x = 'C' + zeroFill(coordinate[1], 8, 16)
          let y = 'R' + zeroFill(-coordinate[2] - 1, 8, 16)
          let z = 'L' + zeroFill(coordinate[0], 2, 10)
          return 'http://172.16.7.11:8043/' + z.toUpperCase() + '/' + y.toUpperCase() + '/' + x.toUpperCase() + '.png' // 这里可以修改地图路径
        },
    ...
    map.on('pointermove', function (evt) {
      if (map.hasFeatureAtPixel(evt.pixel)) {
        ...
        map.getTargetElement().style.cursor = 'pointer' // 添加小手
        try {
          map.getTargetElement().title = features[0].values_.features[0].values_.name
        } catch (error) {
  ...
  _on(event, handler) {
    ...
    map.on(event, function (evt) {
      if (event === 'click' && map.hasFeatureAtPixel(evt.pixel)) {
        let features = map.getFeaturesAtPixel(evt.pixel)
        handler(features)
      }
    })
...
class Marker {
  constructor(map, opts) {
      ...
      return new ol.style.Style({
        image: new ol.style.Icon(
          /** @type {module:ol/style/Icon~Options} */ ({
            ...
            src: src || 'https://webapi.amap.com/theme/v1.3/markers/n/mark_b.png',
            scale: 0.75
          })
        )
      })
    ...
    let point
    point = ol.proj.transform([parseFloat(opts.position.lng), parseFloat(opts.position.lat)], 'EPSG:4326', 'EPSG:3857')
    let geo = new ol.geom.Point(point)
    let iconFeature = new ol.Feature({
      geometry: geo,
      name: opts.title || ''
    })
    iconFeature.setStyle(createStyle(opts.icon, undefined))
    iconFeature.setId(opts.id)
  ...
  let clusters = new ol.layer.Vector({
    ...
    style: function (feature) {
      ...
      if (size === 1) {
        ...
        // console.log(feature.values_.features[0].values_.icon)
...
class MarkerClusterer {
  constructor(map, markers, opts) {
    ...
    map._addLayer(clusters.clusters, 1)
    Object.assign(this, clusters)
    return this
  }
  ...
  _addMarkers(markers) {
    if (markers.length > 0) {
      ...
      for (let i = 0; i < markers.length; ++i) {
        features[i] = markers[i].Feature
      }
      this.clusterSource.getSource().addFeatures(features)
      this.clusters.setSource(this.clusterSource)
    }
  }
  ...
  _removeMarkers(removeMarkers) {
    for (let i = 0; i < removeMarkers.length; i++) {
      ...
      for (let j = 0; j < currentFeatures.length; j++) {
        if (currentFeatures[j].getProperties().id === removeMarkers[i].opts.id) {
