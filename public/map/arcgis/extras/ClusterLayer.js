define([
  ...
  PopupTemplate, GraphicsLayer, PictureMarkerSymbol
) {
  return declare([GraphicsLayer], {
    constructor: function(options) {
      ...
      this.size = this._clusterData[0] ? {
        width: this._clusterData[0].symbol.width,
        height: this._clusterData[0].symbol.height
      } : {
        width: 32,
        height: 32
      }
	...
	// public ClusterLayer methods  kln reCluster
	refresh: function() {
		this._clusterResolution = this._map.extent.getWidth() / this._map.width
        this.clear()
        this._clusterGraphics()
    ...
    _clusterGraphics: function() {
      ...
      for (var j = 0, jl = this._clusterData.length; j < jl; j++) {
		...
		// kln cancel all labels
		if (point && point.attributes && point.attributes.label) {
			point.attributes.label.visible = false
		}
    ...
    _clusterAddPoint: function(p, cluster) {
      ...
      // cluster.attributes.extData = p.attributes.extData
      cluster.attributes.points.push(p.extData)
      // increment the count
    ...
    _clusterCreate: function(p) {
      ...
      var cluster = {
        ...
        'attributes' : {
          ...
          'points': [ p.attributes.extData ]
        },
        'extraData' : p.attributes
      }
    ...
    _showCluster: function(c) {
	  ...
	  // 聚合到一个点的时候，根据imageUrl属性修改图标样式----kln
	  var picSymbol = null
      if (c.attributes.clusterCount === 1) {
        picSymbol = new PictureMarkerSymbol(c.extraData.imageUrl, this.size.width, this.size.height)
        if (c.extraData && c.extraData.label) {
          c.extraData.label.visible = true
        }
      }
    ...
    _addSingles: function(singles) {
      ...
      arrayUtils.forEach(singles, function(p) {
		  // 点击聚合点，根据imageUrl属性显示点样式----kln
		 var picSym = this._singleSym
        if (p.attributes.imageUrl) {
          picSym = new PictureMarkerSymbol(p.attributes.imageUrl, this.size.width, this.size.height)
        }
        var g = new Graphic(
